...
def train(base_model, mt_classifier, generator, iterator, optimizer, optimizer_g, sar_criterion, scheduler, scheduler_g):
    ...
    for data_input, label_input  in Bar(iterator):
        ...
        fake_logits = fake_logits[:, -1]
        sarcasm_probs = torch.softmax(sarcasm_logits[:, :2], dim=1)
        real_logits = sarcasm_logits[:, -1]

        loss_sarcasm = sar_criterion(sarcasm_logits[:,:2], sarcasm_target)
        ...
        total_loss = loss_sarcasm + dics_loss #+ transfer_loss
...
def eval_full(config, loader1, loader2=None, score=False):
    ...
    classifier = modeling.Classifier(in_feature=base_model.output_num(), hidden_size= 512, class_num=3).to(device)
    base_model.load_state_dict(torch.load(f"./ckpts/best_basemodel_sarcasm_{config['args'].lm_pretrained}_{config['args'].lang}_{criterion}_cgan.pth"))
    classifier.load_state_dict(torch.load(f"./ckpts/best_cls_sarcasm_{config['args'].lm_pretrained}_{config['args'].lang}_{criterion}_cgan.pth"))
    ...
    if not score:
        ...
        df[col] = all_outputs
        df = df.astype(int)
        df.to_csv(f'results/{col}_{criterion}_cgan.txt', index=False, header=True)
        ...
        df[col] = all_outputs
        df.to_csv(f'results/{col}_{criterion}_scgan.txt', index=False, header=True)
...
def train_full(config, train_loader, stest_loader):
    ...
    #Instanciate models
    base_model = modeling.TransformerLayerG(pretrained_path=config['pretrained_path'], both=True).to(device)
    mtl_classifier = modeling.Classifier(in_feature=base_model.output_num(), hidden_size= 512, class_num=3).to(device)
    generator = modeling.Generator(in_feature=128, hidden_size=base_model.output_num(), emsize=28).to(device)
    cls = 'ATTClassifier'
    ...
    param_base_cls = [{"params": base_model.parameters(), 'lr':  config['lr']}, {'params': mtl_classifier.parameters(), 'lr':  config['lr']}]
    ...
    scheduler_g = get_linear_schedule_with_warmup(optimizer_g,  num_warmup_steps=0, num_training_steps=num_train_steps)
    # Train model
    ...
    for epoch in range(config['epochs']):
        ...
        if total_val_acc > best_total_val_acc:
            ...
            torch.save(base_model.state_dict(), f"./ckpts/best_basemodel_sarcasm_{config['args'].lm_pretrained}_{config['args'].lang}_{criterion}_cgan.pth")
            torch.save(mtl_classifier.state_dict(), f"./ckpts/best_cls_sarcasm_{config['args'].lm_pretrained}_{config['args'].lang}_{criterion}_cgan.pth")
    ...
    with open(f'reports/report_Sarcasm_{config["args"].lm_pretrained}_{config["args"].lang}_cat_{criterion}_cgan.txt', 'w') as f:
