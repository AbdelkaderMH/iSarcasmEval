...
def predict(base_model, mt_classifier, iterator):
    ...
    base_model.eval()
    mt_classifier.eval()
    #utils.enable_dropout(mt_classifier)
    with torch.no_grad():
...
def score(base_model, mt_classifier, iterator):
    ...
    with torch.no_grad():
        for data_input in Bar(iterator):
            ...
            probs = torch.sigmoid(logits).to(device)
            # compute the loss
            all_outputs.extend(probs.squeeze().int().cpu().numpy())
...
def eval_full(config, loader1, loader2=None, score=False):
    ...
    base_model.load_state_dict(torch.load(f"./ckpts/best_basemodel_sarcasm_{config['args'].lm_pretrained}_{config['args'].lang}_{criterion}_sar.pth"))
    classifier.load_state_dict(torch.load(f"./ckpts/best_cls_sarcasm_{config['args'].lm_pretrained}_{config['args'].lang}_{criterion}_sar.pth"))
...
def train_full(config, train_loader, stest_loader):
    ...
    for epoch in range(config['epochs']):
        ...
        if total_val_acc > best_total_val_acc:
            ...
            torch.save(base_model.state_dict(), f"./ckpts/best_basemodel_sarcasm_{config['args'].lm_pretrained}_{config['args'].lang}_{criterion}_sar.pth")
            torch.save(mtl_classifier.state_dict(), f"./ckpts/best_cls_sarcasm_{config['args'].lm_pretrained}_{config['args'].lang}_{criterion}_sar.pth")
    ...
    with open(f'reports/report_Sarcasm_{config["args"].lm_pretrained}_{config["args"].lang}_{criterion}.txt', 'w') as f:
...
if __name__ == "__main__":
    ...
    if args.phase == 'train':
        if args.lang == 'en':
            train_loader, valid_loader = preprocessing.loadTrainValData2(lang=args.lang, batchsize=args.batch_size, num_worker=0, pretraine_path=config['pretrained_path'])
        elif args.lang == 'ar':
            train_loader, valid_loader = preprocessing.loadTrainValData(lang=args.lang, batchsize=args.batch_size,
        ...
        test_loader = preprocessing.loadTestData(lang= args.lang, batchsize=args.batch_size, num_worker=0, pretraine_path=config['pretrained_path'])
        eval_full(config, loader1=test_loader, loader2=None, score=False)
    else:
        test_loader1 = preprocessing.loadTestData2(column='text_0',lang= args.lang, batchsize=args.batch_size, num_worker=0, pretraine_path=config['pretrained_path'])
        test_loader2 = preprocessing.loadTestData2(column='text_1',lang= args.lang, batchsize=args.batch_size, num_worker=0, pretraine_path=config['pretrained_path'])
